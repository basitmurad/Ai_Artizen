...
class _EnhancedActivityScreenState extends State<EnhancedActivityScreen>
  ...
  Future<void> _loadModuleData() async {
    try {
      ...
      if (user != null) {
        ...
        if (snapshot.exists) {
          Map<String, dynamic> moduleData = Map<String, dynamic>.from(snapshot.value as Map);
          setState(() {
            userCoins = moduleData['coins'] ?? 0;
          });
          print('✅ Loaded module data for ${widget.moduleID}: coins=$userCoins');
        }
      }
    } catch (e) {
  ...
  Future<void> _initializeActivityTracking() async {
    try {
      ...
      if (user != null) {
        ...
        print('📝 Activity tracking initialized for scenario ${widget.scenario.id} in module ${widget.moduleID}');
      }
    } catch (e) {
      print('❌ Error initializing activity tracking: $e');
    }
  ...
  Future<void> _awardCoins(int amount, String reason) async {
    try {
      ...
      print('💰 Coins awarded in ${widget.moduleID}: $amount for $reason (Total: $newCoins)');
    } catch (e) {
      print('❌ Error awarding coins: $e');
    }
  }
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text(
                      '+2 coins earned in ${_getModuleName()}!',
                      style: TextStyle(
                        color: Colors.white,
                        fontWeight: FontWeight.bold,
                        fontSize: 14,
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    _buildSummaryRow('Module', _getModuleName()),
                    _buildSummaryRow('Coins Earned', '${_calculateCurrentCoins()}'),
                  ],
                ),
              ),
  ...
  Future<void> _completeActivityTracking() async {
    try {
      ...
      print('🏆 Activity completed in ${widget.moduleID} - Type: ${widget.scenario.activity?.type}, Passed: $passed, Accuracy: ${finalAccuracy.toStringAsFixed(1)}%');
    } catch (e) {
      print('❌ Error completing activity tracking: $e');
    }
  }
  ...
  Future<void> _updateUserActivityStats() async {
    try {
      ...
      await userStatsRef.update({
        ...
        'lastActivityType': widget.scenario.activity?.type,
        'lastActivityPassed': didPassActivity,
        'overallAccuracy': currentTotalItems + totalItems > 0
            ? ((currentCorrectAnswers + correctAnswersCount) / (currentTotalItems + totalItems)) * 100
            : 0,
      });
